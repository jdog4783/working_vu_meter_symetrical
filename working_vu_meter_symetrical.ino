...
#define LED_PIN   4  //Pin for the pixel strand. Does not have to be analog.
#define LED_TOTAL 6  //Change this to the number of LEDs in your strand.
...
float maxVol = 20;     //Holds the largest volume recorded thus far to proportionally adjust the visual's responsiveness.
float knob = 0;   //Holds the percentage of how twisted the trimpot is. Used for adjusting the max brightness.
...
void Pulse() {
  ...
  fade(.75);   //Listed below, this function simply dims the colors a little bit each pass of loop()
